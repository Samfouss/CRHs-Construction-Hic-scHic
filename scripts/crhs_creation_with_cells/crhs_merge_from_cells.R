...
fusion_comp_from_cell <- function(structure_net_comp1, structure_net_comp2, edge_overlap_result){

  crhs = list()
  nb_crh1 = length(structure_net_comp1$crhs)
  nb_crh2 = length(structure_net_comp2$crhs)
  
  paires.crh <- edge_overlap_result$chev_edge_comp
  
  # On boucle sur les CRHs à apparier
  if(sum(structure_net_comp1$crhs[[1]]$mat_incidence)!=-1 & sum(structure_net_comp2$crhs[[1]]$mat_incidence)!=-1){
    ...
    for (j in seq_len(ncol(paires.crh))) {
      ...
      mat1 = structure_net_comp1$crhs[[ens1]]$mat_incidence
      ...
      name1 = structure_net_comp1$crhs[[ens1]]$name
    ...
    for (i in rest_add1) {
      crhs[[length(crhs)+1]] = list(
        "name" = structure_net_comp1$crhs[[i]]$name,
        "mat_incidence" = structure_net_comp1$crhs[[i]]$mat_incidence
      )
    }
  ...
  }else if(sum(structure_net_comp1$crhs[[1]]$mat_incidence)==-1){
    
    crhs = structure_net_comp2$crhs
    
  }else if(sum(structure_net_comp2$crhs[[1]]$mat_incidence)==-1){
    
    crhs = structure_net_comp1$crhs
