...
edge_identity_overlap_from_cell <- function(structure_net_comp1, structure_net_comp2){
  ...
  if(sum(structure_net_comp1$crhs[[1]]$mat_incidence)==-1){
    ...
    for (i in seq_len(nb_crh1)) {
      ...
      mat1 = structure_net_comp1$crhs[[i]]$mat_incidence
      
      for (j in seq_len(nb_crh2)) {
        
        # On recupère ici la matrice d'incidence
        mat2 = structure_net_comp2$crhs[[j]]$mat_incidence
...
res_over <- edge_identity_overlap_from_cell(results1, results2)
