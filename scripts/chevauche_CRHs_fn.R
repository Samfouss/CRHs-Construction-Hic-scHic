...
chr_overlap <- function(structure_net_comp1, structure_net_comp2){
  ...
  chev_chr_comp1 = rbind((1:structure_net_comp1$no)[structure_net_comp1$csize>1], unlist(c1.list[structure_net_comp1$csize>1]))
  # Composantes du réplicat 2 et leur composante chevauchant le plus dans le réplicat 1
  chev_chr_comp2 = rbind((1:structure_net_comp2$no)[structure_net_comp2$csize>1], unlist(c2.list[structure_net_comp2$csize>1]))
...
edges_overlap <- function(data_frame1, data_frame2, rep1, rep2, edge_var, crh_var){
  ...
  colnames(edges_chev_chr_comp1) <- rownames(chevauche)
  
  # Composantes du réplicat 2 et leur composante chevauchant le plus dans le réplicat 1
  edges_chev_chr_comp2 = rbind(crh_list2, ens2_list)
  colnames(edges_chev_chr_comp2) <- colnames(chevauche)
  
  results = list(
    "chevauche" = chevauche,
    "edges_chev_chr_comp1" = edges_chev_chr_comp1,
    "edges_chev_chr_comp2" = edges_chev_chr_comp2,
